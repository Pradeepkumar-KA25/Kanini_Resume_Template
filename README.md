...
Kanini Resume Builder converts a candidate resume into two standardized Kanini profile formats. Upload a PDF, DOCX, or DOC resume, choose an optional local Ollama model, review generated previews, and download either template as DOCX or PDF.
...
- Generate Kanini Format 1 and Kanini Format 2 profiles as HTML previews, DOCX files, and PDFs.
...
3. The app displays previews for Kanini Format 1 and Kanini Format 2.
4. Download either profile as DOCX or PDF.
...
| `POST` | `/api/upload` | Parse a resume and generate previews/files |
...
| `GET` | `/api/download/{session_id}/{template_id}/{format}` | Download `template1` or `template2` as `docx` or `pdf` |
...
backend/                   FastAPI service, extraction, parsing, rendering, persistence
  main.py                  API orchestration, sessions, validation, enrichment
...
The backend includes script-style verification utilities in `backend/test_*.py`. A formal automated test suite has not yet been established.

Set `PDF_PARSE_DEBUG=1` before starting the backend to write PDF extraction diagnostics to `backend/pdf_parse_debug.log`.

## Packaging

`build_exe.bat` builds the Angular frontend and creates a PyInstaller distribution in `dist/KaniniResumeBuilder`. Before using it on another machine or in CI, update its hard-coded `PYTHON` path for that environment.
